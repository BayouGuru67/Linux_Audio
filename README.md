...
Something that is not so user-friendly about this trend is the widely varying audio levels and quality between the different videos, songs, services, etc. For the HTPC user, this can become a significant issue, greatly affecting one's enjoyment of the Linux multimedia experience. In my HTPC environment where an A/V receiver is amplifying the sound to the speakers and sending the video to the television or monitor, I prefer to keep the computer's output volume set @ 100% (where the signal to noise ratio is the highest), and use the A/V receiver's remote to control the actual volume.  S/N (signal to noise) ratio is a very important number to look at when shopping for audio equipment, and you always want the highest possible number, preferably over 100.  What it represents is simply a measure of a given amount of noise per a given amount of output signal/volume, so if you have a S/N ratio of, say 108 db +/-3 db, then at unity (the 0 db normal maximum output), you should be able to measure at least 105 db of output signal before also measuring an audible amount of noise, meaning the audio should be silent when nothing is playing, not sounding like a balloon with a leak, or a waterfall.  Bear in mind that EVERY piece of analog audio hardware has a S/N ratio and adds noise to any analog signal that passes through it, so adding that 82 db S/N ratio EQ to the analog audio output of your computer will also add a noticeable amount of noise (which is more-easily heard at higher volumes) to the signal going into the amplifier that drives the speakers, which itself also adds more noise....you get the point I hope.  This is where an all-digital signal chain such as using the digital audio output of your HTPC, be it via HDMI or a digital-optical/TOSLink cable, has an advantage over analog in that there is no noise or distortion added to the signal when adding an effect to the computer's audio output or an additional component to the chain such as a digital signal processor or A/V receiver other than any noises the effect or unit itself makes because the signal is all-digital (zeroes and ones) until, in my case, my A/V receiver converts the digital information into an analog signal and feeds it into its own processing and multiple amplifier channels as well as to the preamp outputs feeding my sub.  The higher you like your volume, the more you should appreciate a digital signal chain and higher signal to noise ratios!  This is also why today's concerts sound better than they ever did with analog gear.
